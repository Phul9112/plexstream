...
function PlayerInner() {
  ...
  if (error) return (
    <div style={{ minHeight: '100vh', background: '#000', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 16 }}>
      ...
      <button onClick={() => router.back()} style={{ background: '#e5a00d', color: '#000', borderRadius: 8, padding: '10px 20px', fontWeight: 600, border: 'none', cursor: 'pointer' }}>← Back</button>
    </div>
  );
    ...
    <div style={{ background: '#000', minHeight: '100vh', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }} onMouseMove={resetHideTimer} onClick={resetHideTimer}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: 20, opacity: controls ? 1 : 0, transition: 'opacity 0.3s', pointerEvents: controls ? 'auto' : 'none' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <button onClick={() => router.back()} style={{ background: 'rgba(0,0,0,0.6)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 8, padding: '8px 14px', color: '#fff', fontSize: 13, cursor: 'pointer' }}>← Back</button>
          {title && <span style={{ color: '#fff', fontSize: 15, fontWeight: 500, textShadow: '0 1px 4px rgba(0,0,0,0.8)' }}>{title}</span>}
        </div>
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
          <button onClick={() => seek(-10)} style={{ background: 'rgba(0,0,0,0.6)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 8, padding: '12px 20px', color: '#fff', fontSize: 14, cursor: 'pointer' }}>−10s</button>
          <button onClick={togglePlay} style={{ background: '#e5a00d', border: 'none', borderRadius: 10, padding: '12px 28px', color: '#000', fontSize: 18, fontWeight: 700, cursor: 'pointer' }}>
            {paused ? '▶' : '⏸'}
          </button>
          <button onClick={() => seek(10)} style={{ background: 'rgba(0,0,0,0.6)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 8, padding: '12px 20px', color: '#fff', fontSize: 14, cursor: 'pointer' }}>+10s</button>
        </div>
...
export default function Player() {
  return (
    <Suspense fallback={<div style={{ minHeight: '100vh', background: '#000', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#888' }}>Loading…</div>}>
      <PlayerInner />
    </Suspense>
  );
}
